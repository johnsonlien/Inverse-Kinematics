    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  // and the "euler2Rotation" function.
	...
	for (int i = 0; i < localTransforms.size(); i++) {
		...
		euler2Rotation(jointOrientationEulerAngles[i], R0, rotateOrders[i]);

		localTransforms[i] = RigidTransform4d(asMat3d(R0)*asMat3d(R), translations[i]);		// 4x4 matrix
	}
  ...
  // Also useful are the Mat3d and RigidTransform4d classes defined in the Vega folder.
	for (int i = 0; i < globalTransforms.size(); i++) {
		int child = jointUpdateOrder[i];		// jointUpdateOrder is in hierarchical order already root->leaves
		int parent = jointParents[child];
		if (parent == -1) {	// root
			globalTransforms[child] = localTransforms[child];
		}
		else {
			globalTransforms[child] = globalTransforms[parent] * localTransforms[child];
		}
	}
}
